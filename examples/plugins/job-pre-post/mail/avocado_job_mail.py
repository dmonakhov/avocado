...
class MailCLI(CLI):
    ...
    def configure(self, parser):
        ...
        self.remote_parser.add_argument('--mail-job-report', dest='mail_job_report',
        ...
        self.remote_parser.add_argument('--mail-type', type=str, dest='mail_job_content',
...
class Mail(JobPre, JobPost):
    ...
    def parse_job_opts(self, job):
        ...
        self.server = settings.get_value(section="plugins.job.mail",
                                         ...
                                         key_type=str,
                                         default='localhost')
        self.body_type = settings.get_value(section="plugins.job.mail",
                                         key="content_type",
                                         key_type=str,
                                         default='info')

        if (hasattr(job.args, 'mail_job_report')):
            state = getattr(job.args, 'mail_job_report')
            if state == 'on':
                self.enabled = 1
            if state == 'off':
                self.enabled = 0
        ...
        if hasattr(job.args, 'mail_job_content'):
            if getattr(job.args, 'mail_job_content') is not None:
                self.body_type = getattr(job.args, 'mail_job_content')
    ...
    def _make_stats(self):
        ...
        test1 = os.path.basename(j['tests'][0] ['test'])

        if j['total'] == j['pass']:
            stat = 'ALLPASS (%d/%d) %s' % (j['total'], j['pass'], test1)
        else:
            stat = '[t:%d p:%d e:%d f:%d s:%d] %s ' % (j['total'],
                                            j['pass'],
                                            j['errors'],
                                            j['failures'],
                                            j['skip'],
    ...
    def mail_report(self, job):
        ...
        if self.body_type == 'minimal':
            ...
            try:
                ...
                self.subject = '%s [%s][%s] ' % (self.subject,
                                                 job.unique_id[:7],
                                                 status)
                fp.close()
        ...
        try:
            ...
            smtp.quit()
            self.log.info("MAIL_SUB   : %s" % msg['Subject'])
        except:
